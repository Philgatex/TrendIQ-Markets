...
def normalize_probabilities(bullish_score: float, bearish_score: float, sideways_score: float) -> dict:
    ...
    if total <= 0:
        return {"bullish_probability": 33.3, "bearish_probability": 33.3, "sideways_probability": 33.4}
    bullish = round((bullish_score / total) * 100, 1)
    bearish = round((bearish_score / total) * 100, 1)
    sideways = round(100 - bullish - bearish, 1)
    return {"bullish_probability": bullish, "bearish_probability": bearish, "sideways_probability": sideways}


def generate_prediction(df: pd.DataFrame, regime: dict, market_type: str = "index") -> dict:
    if df is None or df.empty or len(df) < 50:
        return {"final_bias": "No Data", "confidence_score": 0, "bullish_probability": 0, "bearish_probability": 0, "sideways_probability": 0, "explanations": ["Not enough data to generate prediction."], "levels": {}}
    latest = df.iloc[-1]
    ...
    if market_type in ["index", "crypto"]:
        ...
        if market_regime == "Risk-Off" and close > ema20:
            bullish_score += 1
    ...
    probabilities = normalize_probabilities(bullish_score, bearish_score, sideways_score)
    ...
    max_probability = max(bullish_probability, bearish_probability, sideways_probability)
    ...
    return {"final_bias": final_bias, "confidence_score": confidence_score, "bullish_probability": bullish_probability, "bearish_probability": bearish_probability, "sideways_probability": sideways_probability, "explanations": explanations, "levels": trade_levels}
