...
def build_market_regime(snapshot_df: pd.DataFrame) -> dict:
    if snapshot_df is None or snapshot_df.empty:
        return {"market_regime": "No Data", "risk_on_probability": 0, "risk_off_probability": 0, "summary": "No market snapshot available.", "table": pd.DataFrame()}
    df = snapshot_df.copy()
    df["Signal"] = df.apply(lambda row: classify_change(row["% Change"], row["Type"]), axis=1)
    risk_on_score = 0
    risk_off_score = 0
    explanations = []
    us_indices = df[df["Market"].isin(["Nasdaq-100 / US100 Proxy", "S&P 500 / US500 Proxy", "Dow Jones / US30 Proxy", "Russell 2000 Proxy"])]
    global_indices = df[df["Market"].isin(["FTSE 100", "Nikkei 225", "Hang Seng", "DAX Germany"])]
    ...
    display_df = df[["Market", "Symbol", "Type", "Price", "% Change", "Signal"]].copy()
    return {"market_regime": market_regime, "risk_on_probability": risk_on_probability, "risk_off_probability": risk_off_probability, "summary": " ".join(explanations), "table": display_df}
