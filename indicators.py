...
def detect_support_resistance(df: pd.DataFrame, lookback: int = 50) -> dict:
    if df is None or df.empty:
        return {"support": None, "resistance": None, "recent_low": None, "recent_high": None}
    ...
    return {"support": support, "resistance": resistance, "recent_low": recent_low, "recent_high": recent_high}


def get_indicator_summary(df: pd.DataFrame) -> dict:
    if df is None or df.empty:
        return {"trend": "No Data", "rsi_status": "No Data", "macd_status": "No Data", "volatility_status": "No Data"}
    latest = df.iloc[-1]
